...
class Cart
{
    ...
    public function add($item, $id){
        $storedItem = ['qty' => 0, 'price'=> $item->supplement_price, 'item' => $item]; // group
        if($this->items){
        ...
        $storedItem['qty']++;
        $storedItem['price'] = $item->supplement_price * $storedItem['qty'];
        $this->items[$id] = $storedItem; // add the storedItem to the items array 
        $this->totalQty++;
        $this->totalPrice += $item->supplement_price;
    }
